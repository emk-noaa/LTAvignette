...
You can then pass this list of edits to `das_editor()` (next subsection on this page), or as an argument within `process_surveys()` (next page).  However, for the sake of full code reproducibility, it may be most useful to use this output to write coded edits for us with `process_surveys()` (or `das_editor()`) as demonstrated below.   
