...
To validate our results and the routines within `LTabundR`, we will compare the `LTabundR` WHICEAS results to those from A.L. Bradford et al. (2021). To do so, we first bring in the ALB et al. (2021) results, which we've staged in a GoogleSheet provided in the code chunk below: 
...
Format these data to facilitate joining and analyzing alongside our new WHICEAS results within a `tidyverse` framework: 
...
whiceas_join <- function(alb, ltas){
  ...
  for(i in 1:length(ltas)){
    ...
    ltabundi <-
      left_join(lti$estimate,
                lti$bootstrap$summary %>% select(title, Region, year, species, iterations, g0_cv, CV, L95, U95),
  ...
  ltabundr <- ltabundr %>%
    mutate(year = as.numeric(year),
           D = D*1000,
           N = round(N)) %>%
    select(title, species, year, g0=g0_est, g0_cv, ESW = ESW_mean, ss = size_mean, D, N, CV, L95, U95)
...
Next, we will write a few helper functions. First, a function that joins the ALB et al. (2021) results together to our new estimates. 

Then a set of plotting functions: 
...
# Density comparison
whiceas_d <- function(mr){
    ggplot(mr %>% mutate(year = factor(year)),
           aes(x=D, y=alb_D, color=title, shape=year)) +
    geom_point() +
    ...
    xlab('LTabundR') + ylab('ALB et al. (2021)') +
    labs(title='Density')
}

# Abundance comparison
whiceas_n <- function(mr){
    ggplot(mr %>% mutate(year = factor(year)),
           aes(x=N, y=alb_N, color=title, shape=year)) +
    geom_point() +
    scale_x_continuous(trans='log', limits=c(10, 30000), 
                       labels = function(x)round(x, 2)) +
    scale_y_continuous(trans='log', limits=c(10, 30000), 
                       labels = function(x)round(x, 2)) +
    geom_abline(slope = 1, intercept = 0, lty=3, alpha=.6) +
    xlab('LTabundR') + ylab('ALB et al. (2021)') +
...
whiceas_lci <- function(mr){
    ggplot(mr %>% mutate(year = factor(year)),
           aes(x=L95, y=alb_L95, color=title, shape=year)) +
    geom_point() +
    scale_x_continuous(trans='log', labels = function(x)round(x, 2)) +
    scale_y_continuous(trans='log', labels = function(x)round(x, 2)) +
    geom_abline(slope = 1, intercept = 0, lty=3, alpha=.6) +
    xlab('LTabundR') + ylab('ALB et al. (2021)') +
    labs(title='Lower 95% CI of abundance')
}

whiceas_uci <- function(mr){
    ggplot(mr %>% mutate(year = factor(year)),
           aes(x=U95, y=alb_U95, color=title, shape=year)) +
    geom_point() +
    scale_x_continuous(trans='log', labels = function(x)round(x, 2)) +
    scale_y_continuous(trans='log', labels = function(x)round(x, 2)) +
    geom_abline(slope = 1, intercept = 0, lty=3, alpha=.6) +
    xlab('LTabundR') + ylab('ALB et al. (2021)') +
    labs(title='Upper 95% CI of abundance')
}

# School size comparison
whiceas_ss <- function(mr){
  ggplot(mr %>% mutate(year = factor(year)),
           aes(x=ss, y=alb_ss, color=title, shape=year)) +
    geom_point() +
    scale_x_continuous(trans='log', limits=c(1, 300), labels = function(x)round(x, 2)) +
    scale_y_continuous(trans='log', limits=c(1, 300), labels = function(x)round(x, 2)) +
    geom_abline(slope = 1, intercept = 0, lty=3, alpha=.6) +
    xlab('LTabundR') + ylab('ALB et al. (2021)') +
    labs(title='School size')
}

# g(0) comparison
whiceas_g0 <- function(mr){
    ggplot(mr %>% mutate(year = factor(year)),
           aes(x=g0, y=alb_g0, color=title, shape=year)) +
    geom_point() +
    geom_abline(slope = 1, intercept = 0, lty=3, alpha=.6) +
    xlab('LTabundR') + ylab('ALB et al. (2021)') +
    labs(title='g(0)')
...
```{r, echo=FALSE, eval=FALSE, collapse=TRUE, message=FALSE, warning=FALSE}
################################################################################
# Manual g0 based on ALB et al. (2021) manuscript

ltas <- lta_enlist('../test_code/eric/whiceas/lta_manual_g0/')
plots <- results_compare(ltas)
plots$ss %>% ggplotly()
plots$g0 %>% ggplotly()
plots$esw %>% ggplotly()
plots$d %>% ggplotly()
...
################################################################################
# Auto g0 based on barlow2015

#ltas <- lta_enlist('../LTabundR-dev/test_code/eric/whiceas/lta_barlow2015_g0/')
ltas <- lta_enlist('../test_code/eric/whiceas/lta_barlow2015_g0/')
plots2 <- results_compare(ltas)
plots2$ss %>% ggplotly()
plots2$g0 %>% ggplotly()
plots2$esw %>% ggplotly()
plots2$d %>% ggplotly()
...
################################################################################
# Auto g0 based on new Rg0

ltas <- lta_enlist('../test_code/eric/whiceas/lta/')
plots3 <- results_compare(ltas)
plots3$ss %>% ggplotly()
plots3$g0 %>% ggplotly()
plots3$esw %>% ggplotly()
plots3$d %>% ggplotly()
...
################################################################################
# Show N estimates in three panels:
# with common legend

df <- plots$n$data %>% mutate(version = 'g(0) from ALB et al. (2022)')
df2 <- plots2$n$data %>% mutate(version = 'g(0) by LTabundR with Barlow (2015) data')
df3 <- plots3$n$data %>% mutate(version = 'g(0) by LTabundR with 1986-2020 data')
dfs <- rbind(df, df2, df3)
head(dfs)

ggplot(data=dfs, aes(x=N, y=alb_N, color=title, pch=year)) +
  scale_x_continuous(trans='log', breaks=c(10, 100, 500, 1000, 5000, 10000, 50000, 100000, 500000, 1000000)) +
  scale_y_continuous(trans='log', breaks=c(10, 100, 500, 1000, 5000, 10000, 50000, 100000, 500000, 1000000)) +
  xlab('LTabundR') + ylab('ALB et al. (2022)') +
  geom_point(alpha=.7) +
  geom_abline(slope=1, intercept=0, lty=3, alpha=.5) +
  facet_wrap(~version)
```
