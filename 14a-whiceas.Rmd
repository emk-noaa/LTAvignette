...
```{r, echo=FALSE, eval=FALSE, collapse=TRUE, message=FALSE, warning=FALSE, fig.heigh=9, fig.width=8}
# scratchpad
...
data(land)
landnew <- st_transform(land, crs= 4326)

data(eez_hawaii)
st_crs(eez_hawaii) <- 4326

data(strata_cnp)
whiceas_poly <- process_polygon(strata_cnp$WHICEAS)$sf
st_crs(whiceas_poly) <- 4326

bottlenose1 <- process_polygon(strata_cnp$Bottlenose_KaNi)$sf
st_crs(bottlenose1) <- 4326
bottlenose2 <- process_polygon(strata_cnp$Bottlenose_OUFI)$sf
st_crs(bottlenose2) <- 4326
bottlenose3 <- process_polygon(strata_cnp$Bottlenose_BI)$sf
st_crs(bottlenose3) <- 4326

spotted1 <- process_polygon(strata_cnp$Spotted_OU)$sf
st_crs(spotted1) <- 4326
spotted2 <- process_polygon(strata_cnp$Spotted_FI)$sf
st_crs(spotted2) <- 4326
spotted3 <- process_polygon(strata_cnp$Spotted_BI)$sf
st_crs(spotted3) <- 4326
...
ggplot(data = land) +
    geom_sf(fill='darkslategrey', color=NA, alpha=.6)  +
    geom_sf(data=eez_hawaii, color='grey70', size=.2) + 
    geom_sf(data=bottlenose1, size=.5, alpha=0, color=adjustcolor('firebrick', alpha=.5)) + 
    geom_sf(data=bottlenose2, size=.5, alpha=0, color=adjustcolor('firebrick', alpha=.5)) + 
    geom_sf(data=bottlenose3, size=.5, alpha=0, color=adjustcolor('firebrick', alpha=.5)) + 
    geom_sf(data=spotted1, size=.5, alpha=0, color=adjustcolor('forestgreen', alpha=.5)) + 
    geom_sf(data=spotted2, size=.5, alpha=0, color=adjustcolor('forestgreen', alpha=.5)) + 
    geom_sf(data=spotted3, size=.5, alpha=0, color=adjustcolor('forestgreen', alpha=.5)) + 
    geom_sf(data=whiceas_poly, size=.8, alpha=0, color=adjustcolor('steelblue4', alpha=.5)) + 
    coord_sf(crs=4326, xlim=c(-162, -153), ylim=c(17, 24.2)) + 
  ...
  # Effort
  geom_point(data=cruz1720$cohorts$all$das %>% filter(Bft <= 6,
                                                      EffType == 'S',
                                                      OnEffort == TRUE, 
                                                      use == TRUE),
             mapping=aes(x=Lon,
             ...
             alpha=.1) + 
  # Sightings
  geom_point(data=cruz1720$cohorts$all$sightings %>% filter(included == TRUE),
             mapping=aes(x=Lon,
                         y=Lat)) + 
  facet_wrap(~year, ncol=2)

ggsave(filename = 'whiceas/map.pdf', width=9, height=4.5)


# Sperm whale encounter rate analysis ==========================================
er_results <- 
  er_simulator(spp = '046', cohort = 1, cruz = cruz1720, iterations = 1000)
er_results$summary

# Short-finned pilot whale encounter rate analysis ==========================================
er_results <- 
  er_simulator(spp = '036', cohort = 1, cruz = cruz1720, iterations = 1000)
er_results$summary
...
```{r, echo=FALSE, eval=FALSE, collapse=TRUE, message=FALSE, warning=FALSE, fig.heigh=9, fig.width=8}
# scratchpad
#pdf(file='whiceas/df.pdf', width=6, height=6)
g0_plot(Rg0, panes = 3)
ggsave(filename='whiceas/Rg0.pdf',width=6,height=8)
#dev.off()

Rg0 %>% 
  group_by(title) %>% 
  summarize(n = sum(sits)) %>% 
  as.data.frame
...
# ==============================================================================
# Fix confidence intervals -- run this once -- 
# It will read in each result, correct CIs, then re-save the RData object
if(FALSE){
  result_paths <- c('whiceas/lta_manual/',
                    'whiceas/lta_barlow/',
                    'whiceas/lta/')
  p <- i <- 1
  for(p in 1:length(result_paths)){
    
    (resulti <- result_paths[p])
    message(resulti)
    (diri <- paste0(resulti,dir(resulti)))
    for(i in 1:length(diri)){
      (lti_fn <- diri[i])
      message('--- ',lti_fn)
      lti <- readRDS(lti_fn)
      lti %>% names
      
      # Harvst parts of results
      (esti <- lti$estimate)
      (summi_old <- lti$bootstrap$summary)
      booti <- lti$bootstrap$details
      booti <- left_join(booti,
                         esti %>% select(title, Region, year, Nmean=N))
      
      # Replace confidence intervals
      summi <- 
        booti %>% 
        group_by(title, year) %>% 
        summarize(Nmeani = round(Nmean[1]),
                  LCI = round(lta_ci(Nmeani, N)$bca_lognormal[1]),
                  UCI = round(lta_ci(Nmeani, N)$bca_lognormal[2])) %>% 
        rename(N = Nmeani)
      summi
      
      # Make new summary object
      summi_new <- left_join(summi_old %>% select(title:CV),
                             summi %>% select(title, year, L95=LCI, U95=UCI))
      summi_new %>% as.data.frame
      
      # Replace summary in lti
      lti$bootstrap$summary <- summi_new
      
      # Resave RData object
      saveRDS(lti, file=lti_fn)
...
ltas[[1]]$bootstrap$summary # confirm 1000 iterations
reporti <- lta_report(ltas, cruz)
reporti$table3[,c(1,4,5, 8,9)]
reporti$table4 %>% head
reporti$table4[,c(1,2,4,6,8)]
...
# LTabundR
ltas <- lta_enlist('whiceas/lta/')
ltas[[1]]$bootstrap$summary %>% as.data.frame() # confirm 1000 iterations
reporti <- lta_report(ltas, cruz)
reporti$table3[,c(1,4,5, 8,9)]
reporti$table4[,c(1,2,4,6,8)]
reporti$table4[,c(1,3,5)] # 2017
reporti$table4[,c(1,7,9)] # 2020

# Bootstrap histograms
mri <- ltas[[3]]$bootstrap$details
ggplot(mri %>% filter(year==2020), 
       aes(x=N)) + 
  geom_histogram() + 
...
for(i in 1:length(ltas)){
  lti <- ltas[[i]]
  lti$estimate %>% head
  lte <- lti$estimate %>% select(title, species, Region, year, D, N)
  ltb <- lti$bootstrap$summary %>% select(title, Region, year, L95, U95) 
  
  ltb95 <- left_join(lte, ltb)
  ltb95
  # ltb95 <- 
  #   ltb %>% 
  #   #filter(N < 300000) %>% 
  #   group_by(title, year) %>% 
  #   summarize(L95 = lta_ci(Nmean, N)
  #               coxed::bca(N)[1],
  #             U95 = coxed::bca(N)[2])
  # ltb95
  # lte <- left_join(lte, ltb95) #, by=c('title','year'))
  ests <- rbind(ests, ltb95)
}
nrow(ests)
ests %>% head

ggplot(ests %>% 
         mutate(title = gsub('Unidentified','Unid.',title)) %>% 
         mutate(title = gsub('Pantropical','',title)) %>% 
         mutate(title = gsub('spotted','Spotted',title)) %>% 
         group_by(title) %>% 
         mutate(both = all(N > 1)) %>% 
         ungroup() %>% 
         filter(both == TRUE) %>% 
         filter(!grepl('Unid.', title))
       ) +
  geom_point(mapping = aes(x=year, y=N)) + 
  geom_segment(mapping = aes(x=year, xend = year, y=L95, yend=U95)) + 
...
ggsave(filename='whiceas/abundance.pdf',width=8, height=6)
...
mr_plot <- 
  mr %>% 
  mutate(title_short = gsub('Unidentified','Unid.', title)) %>% 
  mutate(title_short = gsub('dolphin','d.', title_short)) %>% 
  mutate(title_short = gsub('Pantropical','', title_short)) %>% 
  mutate(title_short = gsub('spotted','Spotted', title_short)) %>% 
  mutate(title_short = gsub('whale','w.', title_short))
...
ggplot(mr_plot,
       aes(x=alb_D, y=D, 
           color=factor(year))) + 
  geom_point(size=2) +
  ggrepel::geom_text_repel(mapping=aes(label = title_short),
                           size=2.5,
  ...
  labs(title='Density (cetaceans per 1,000 square km)', 
       shape='Year', 
       color = 'Year') + 
...
ggsave(filename='whiceas/compare_density.pdf', 
       width= pdf_width, 
       height=pdf_height)
...
ggplot(mr_plot,
  ...
  scale_x_continuous(trans='log', 
                       limits=c(10, 50000), 
                       breaks = c(10, 25, 100, 250, 1000, 2500, 10000, 50000),
  ...
  ylab('LTabundR') + xlab('ALB et al. (2022)') +
  facet_wrap(~scenario, nrow=3) + 
  labs(title='Abundance in WHICEAS study area', 
       shape='Year', 
       color = 'Year') + 
  scale_color_manual(values=c("grey50", "black")) +
  theme_light() + 
...
ggsave(filename='whiceas/compare_abundance.pdf', 
       width= pdf_width, 
       height=pdf_height)
...
ggplot(mr_plot,
  ...
  scale_y_continuous(limits=c(-.1,1.1), breaks=seq(0, 1, by=.1)) + 
  theme(strip.background = element_rect(fill = "grey45"),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank())

ggsave(filename='whiceas/compare_g0.pdf', 
       width= pdf_width, 
       height=pdf_height)
