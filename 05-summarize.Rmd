...
```{r, echo=TRUE, eval=TRUE, collapse=TRUE, include=TRUE}
bft <- summarize_bft(cruz, cohort=1)
```
...
The slot `$by_year` provides the above for each year separately (note that our example data have only one year):
...
```{r, echo=TRUE, eval=TRUE, collapse=TRUE, include=TRUE}
sightings <- summarize_sightings(cruz,
                                 cohort=1)
```
...
```{r, echo=TRUE, eval=TRUE, collapse=TRUE, include=TRUE}
species <- summarize_species(spp='046', cruz)
```
