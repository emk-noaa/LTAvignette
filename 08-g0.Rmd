...
```{r, echo=TRUE, eval=FALSE, collapse=TRUE, include=TRUE}
cnp <- cnp_1986_2020_10km
cnp$cohorts$all$segments$seg_id <- paste0('cnp_', cnp$cohorts$all$segments$seg_id)
cnp$cohorts$all$sightings$seg_id <- paste0('cnp_', cnp$cohorts$all$sightings$seg_id)
cnp$cohorts$all$das$seg_id <- paste0('cnp_', cnp$cohorts$all$das$seg_id)

ccs <- ccs_1986_2020_10km
ccs$cohorts$all$segments$seg_id <- paste0('ccs_', ccs$cohorts$all$segments$seg_id)
ccs$cohorts$all$sightings$seg_id <- paste0('ccs_', ccs$cohorts$all$sightings$seg_id)
ccs$cohorts$all$das$seg_id <- paste0('ccs_', ccs$cohorts$all$das$seg_id)

# Combine
cruz <- cnp
cruz$cohorts$all$segments <- rbind(cruz$cohorts$all$segments,
                                   ccs$cohorts$all$segments)
cruz$cohorts$all$sightings <- rbind(cruz$cohorts$all$sightings,
                                    ccs$cohorts$all$sightings)
cruz$cohorts$all$das <- rbind(cruz$cohorts$all$das,
                              ccs$cohorts$all$das)
```
